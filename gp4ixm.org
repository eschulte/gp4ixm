...
* Issues / Notes
** TODO next steps [0/6]
*** TODO fix resetting sharing rate w/R packet
...
#+begin_src clojure :var ind=best-ind-big-grid-sine :session
  (let [operators '(\+ \- \* \/)
        stack '()
        [member [x seq]
         (if (seq sq)
           (if (= x (first sq))
             ...
             (recur x (rest sq))))]
        [eat (fn [stack el]
               (if (member el operators)
                 (eat (cons (str "( " (second stack) el (first stack)) (rest (rest (rest stack))))
                      (first stack))))]])
#+end_src
