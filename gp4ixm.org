...
1) run the noco-evo test suite
2) recompile coevolution/sketch.pde with the =#define COEVOLUTION=
   switch set
3) run the co-evo test suite

all with with the following shell command
#+begin_src sh
  ruby coexperimenter.rb /experiments/noco-evo && cd coevolve && make && sleep 300 && cd ../ && ruby coexperimenter.rb /experiments/co-evo
#+end_src
the audacity!
...
*** DONE make 100*sin(x) the default startup goal.
*** DONE confirm that score/evaluation is doubles all the way through
still looks like we're getting integer scores out the back end

*** DONE port the sine functionality to coevolution sketch
*** DONE modest coevolution GP parameters
at least an order of magnitude greater than individual evolution

*** DONE modest coevolution mutation
by single values rather than random large jumps
*** DONE allow reset of eval_GP params with reset packet
these could use capital letters rather than lower case

*** TODO run large test suite w/coevolution

1) first this will require a copy of the experimenter
2) then to select the eval_GP params to be tweaked
3) finally to run and collect data

alright, the coevolve.rb script has been written, now to run it
overnight at school

*** TODO run large test suite w/evolve
this has been done before, but now do a definitive one.
